...
char **query(char *s, int *marime, int *cap)
{
    int i, nr_cuv=0, sec;
    char **cuv, *p;
...
int main()
{
    ...
    int i, j, size=0, cap=3, marime, k, ok,capa;
    ...
    cuvinte=query(lista, &marime,&capa);
    k=0;
    for(i=0;i<size;i++)
    {
        ok=1;
        ...
        else
        {
            for(j=0;j<marime;j++)
            {
                //Daca nu are un cuvant cu -, verificam daca are cuvinte sau o secventa de sine statatoare
